...
local Window = Rayfield:CreateWindow({
   Name = "DarkPegasus DTNB Cheat",
   LoadingTitle = "DarkPegasus Script",
   LoadingSubtitle = "by DarkPegasus",
   ConfigurationSaving = {
      Enabled = true,
   ...
   KeySettings = {
      Title = "DarkPegasus",
      Subtitle = "Key System",
      Note = "Roblox Name Mythicgalaxys (DarkPegasus)",
      FileName = "DarkPegasusKey",
      SaveKey = true,
...
local JumpSlider = Character:CreateSlider({
   Name = "JumpPower",
   Range = {0, 300},
   Increment = 10,
   Suffix = "Jump Hack",
...
local HealtSlider = Character:CreateSlider({
   Name = "Health",
   Range = {0, 100},
   Increment = 10,
   Suffix = "Health Hack",
...
local LastRefresh = 0;
local OptionsFile = 'DarkPegasus_ESP_SETTINGS.dat';
local Binding = false;
local BindedKey = nil;
local OIndex = 0;
local LineBox = {};
...
shared.RSName = shared.RSName or ('DarkPegasusEsp_by_Dark-' .. HttpService:GenerateGUID(false));
...
local Dex = Otr:CreateButton({
   Name = "Dark Dex",
   Callback = function()
      loadstring(game:HttpGetAsync("https://pastebin.com/raw/fPP8bZ8Z"))()
   end,
})
local SecDex = Otr:CreateButton({
   Name = "Secure Dex",
   Callback = function()
